...
#[ test ] fn test_userset()
{
	...
	assert_eq!( cfg.userset().unwrap().jptr( "/my_app/log_lvl"  ).unwrap(), "error" );
	assert_eq!( cfg.userset().unwrap().jptr( "/other_comp/algo" ).unwrap(), "Gauss" );
...
#[ test ] fn test_settings()
{
	...
	assert_eq!( cfg.get().other_comp.algo, "Gauss"          );
